...
for currCon = 2:8
    ...
    for j = conStart:conEnd
        ...
        if usePercent == true
            edgeDists(jIdx) = (edgeDists(jIdx)-baseHeight)/baseHeight*100;
        end
    end
    ...
    relDats(enterIdx, 2) = maxEdge;%(time/fps)*1000;
    ...
    relDats(enterIdx, 5) = time;
    
    relDats(enterIdx, 6) = size(cData{llane}{celll},1);
end